 ...
 * `{ ok: false }` (see `TapExecResult`); an actual throw is a binding bug.
 */

// Bump ONLY on incompatible schema-shape changes; adding commands or params
// never requires a bump. The CLI rejects versions it does not understand.
...
// The wire envelope `exec` resolves with. Any `ok: false` (dispatch or domain
